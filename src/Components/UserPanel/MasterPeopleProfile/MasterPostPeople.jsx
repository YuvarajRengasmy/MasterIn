...
import { getFilterPostUser, getSinglePost, saveLikes, savePostComments, updateBlockPost, updatePostReport } from '../../../api/post';
...
const MasterPostPeople = ({ id }) => {
    ...
    const getPostList = () => {
        const data = { user: id }
        getFilterPostUser(data).then(res => {
            ...
            <div className='scroll-bar' style={{ overflow: 'auto', maxHeight: 'calc(100vh - 50px)' }}>
                <div className='container mb-5' >
                    ...
                    <div className='gap-3 d-flex flex-column' style={{ marginTop: '4.5rem' }}>
                                        ...
                                        <div className='col-lg-1 col-2'>
                                            <div className="dropdown">
                                                ...
                                                <ul className="dropdown-menu" style={{ right: '0', left: 'unset' }}>
                                                    <li><Link className="dropdown-item d-flex align-items-center" onClick={(event) => {event.preventDefault(); openHidePopup(data?._id)}}><FiAlertCircle className='me-2' /> Hide</Link></li>
                                                    <li><Link className="dropdown-item d-flex align-items-center" onClick={(event) => {event?.preventDefault(); openReportPopup(data?._id)}} ><MdHideSource className='me-2' /> Report</Link></li>
                                                </ul>
                                            </div>
                        ...
                        {post?.length === 0 && <div className='text-center text-danger'>No Post.</div>}
                    </div>
                </div>
            </div>
            {/* </div>
...
export default MasterPostPeople;